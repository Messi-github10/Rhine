...
/// CPU software decoder using avcodec_send_packet / avcodec_receive_frame.
class CpuDecoder : public IDecoder {
public:
    explicit CpuDecoder(const DecoderConfig &config);
    ~CpuDecoder() override;

    CpuDecoder(const CpuDecoder &) = delete;
    CpuDecoder &operator=(const CpuDecoder &) = delete;
